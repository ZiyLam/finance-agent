...
def create_futu_market_data_tool(client: "FutuClient") -> FunctionTool:
    """Expose bounded read-only Futu OpenD market-data requests to a model."""

    from .market_data.futu import FutuError
    ...
    def market_data(arguments: Mapping[str, Any]) -> str:
        ...
        if action == "historical_candles":
            ...
            if not isinstance(interval, str) or not isinstance(autype, str):
                raise ValueError("'interval' and 'autype' must be strings")
            limit = arguments.get("limit", 120)
            if not isinstance(limit, int) or isinstance(limit, bool) or not 1 <= limit <= 120:
                raise ValueError("'limit' must be an integer between 1 and 120")
            try:
            ...
            except FutuError as error:
                return f"ERROR: {error}"
            return json.dumps(
                {
                    "source": "Futu OpenAPI (FutuOpenD)",
                    "code": code,
                    "interval": interval,
                    "autype": autype,
                    "returned_rows": len(candles),
                    ...
                    "candles": [
                        {
                            ...
                            "pe_ratio": str(candle.pe_ratio) if candle.pe_ratio is not None else None,
                        }
                        for candle in candles[-limit:]
                    ],
                },
        ...
        if action == "market_snapshot":
            ...
            if (
                ...
                or not 1 <= len(codes) <= 20
                or not all(isinstance(code, str) for code in codes)
            ):
                raise ValueError("'codes' must be a list of 1 to 20 code strings")
            try:
                snapshots = client.market_snapshot(codes)
            except FutuError as error:
                return f"ERROR: {error}"
            return json.dumps(
                {
                    ...
                    "snapshots": [
                        {
                            ...
                            "low": str(snapshot.low_price) if snapshot.low_price is not None else None,
                            "previous_close": str(snapshot.previous_close)
                            ...
                            if snapshot.turnover is not None
                            else None,
                            "turnover_rate_percent": str(snapshot.turnover_rate_percent)
                            if snapshot.turnover_rate_percent is not None
                            else None,
                            "pe_ratio": str(snapshot.pe_ratio)
                            if snapshot.pe_ratio is not None
                            else None,
                            "pb_ratio": str(snapshot.pb_ratio)
        ...
        raise ValueError("'action' must be 'historical_candles' or 'market_snapshot'")

    return FunctionTool(
        name="futu_market_data",
        description=(
            "Reads market data through an operator-started, logged-in local FutuOpenD gateway. "
            "Actions: historical_candles for one Futu code such as HK.00700, US.AAPL, or "
            "SH.600519 (start_date/end_date YYYY-MM-DD, interval 'd'/'w'/'m', autype "
            "'qfq'/'hfq'/'none', optional limit 1-120); and market_snapshot for 1-20 Futu codes. "
            "Quote permissions and delay depend on the connected Futu account. Read-only; never trades."
        ),
        handler=market_data,
    )
