...
本地 Kind 的凭证文件格式、同步、轮换和服务器迁移边界见 [凭证生命周期规范](docs/credentials.md)。
...
微信小程序当前为**兼容维护状态**：暂不新增页面、交互或业务能力，也不作为当前发布目标；目录和后端路由继续保留。每次 Web/API 变更仍须保持设计令牌同步，并由契约测试确认小程序现有调用继续存在于同一份 OpenAPI 中。恢复支持前，再集中进行微信开发者工具编译、真机适配与发布合规验证。
